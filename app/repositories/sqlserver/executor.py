        ...
        *,
        ...
        try:
            ...
            logger.error(
                ...
                extra={"procedure_name": procedure_name, "error": str(exc)},
                exc_info=exc,
            )
        ...
        try:
            ...
            # Anything caught here is NOT a pyodbc/SQL Server error (those are
            # handled above) -- it's a bug in this executor or a Python-level
            # failure while processing the result set. Log the real exception
            # type/message/traceback so the next failure isn't invisible in
            # `docker logs` like this one was.
            logger.error(
