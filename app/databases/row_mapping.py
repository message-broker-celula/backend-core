...
all return the same row shape (Spanish column names, e.g. `id_bd`,
`nombre_bd`, `estado`). This module is the single place that knows those
column names, so `DatabaseService` and `AdminService` never have to agree on
